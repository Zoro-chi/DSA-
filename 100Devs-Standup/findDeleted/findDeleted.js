...
var findDeletedNumber = function (arr, mixArr) {
    if (arr.length === mixArr.length) { /* O(n) : Linear */
        return 0;
    }
    else {
        var arrSum = arr.reduce(function (a, c) { return a + c; }, 0);
        var mixSum = mixArr.reduce(function (a, c) { return a + c; }, 0); /* O(1) : Constant */
